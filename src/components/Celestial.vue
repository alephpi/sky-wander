...
import { extent, geoEquirectangular, geoPath, scalePow, scaleQuantize } from 'd3'
...
function render() {
  ...
  const { ctx, dpi } = initCanvas(canvas, length, length)
  ...
  const adapt: number = Math.sqrt(projection.scale() / scale)
  const starSizeRange = scalePow(starMagRange, starMagRange.map(starSize)).exponent(1)
  // projection.translate([width / 2, height / 2]).scale(scale * zoomlevel)
  projection.fitExtent([[0, 0], [width, height]], stars)
  // console.log(proj.scale())

  const center = [width / 2, width / 2]
  const half_edge = width / (2 * Math.sqrt(2))

  const top_left = [center[0] - half_edge, center[1] - half_edge]
  ...
  // ctx.fillStyle = '#FFF'
  // ctx.fillRect(top_left[0], top_left[1], 2 * half_edge, 2 * half_edge)
